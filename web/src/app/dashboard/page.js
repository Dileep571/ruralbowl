...
function PlanCard({ plan, onSubscribe, loading }) {
  return (
    <div className="border rounded-xl p-5 flex flex-col">
      ...
      <div className="mt-3 text-2xl font-bold text-green-700">₹{Number(plan.price || 0).toFixed(2)}<span className="text-sm font-normal text-gray-500">/{plan.interval || 'month'}</span></div>
      <ul className="mt-3 text-sm text-gray-700 list-disc list-inside space-y-1">
        {Array.isArray(plan.features) ? plan.features.map((f, i) => <li key={i}>{f}</li>) : null}
      </ul>
      <button
        onClick={() => onSubscribe(plan.id)}
        disabled={loading}
        className="mt-4 inline-flex items-center justify-center px-4 py-2 rounded-lg bg-green-600 text-white hover:bg-green-700 disabled:opacity-50"
      >
